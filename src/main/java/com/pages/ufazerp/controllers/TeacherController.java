...
public class TeacherController {
    ...
    public ResponseEntity<Object> getAllSubjectsByTeacherId(@PathVariable("id") long id) {
        try {
            return ok(json("subjects", teacherService.readAllByTeacherId(id).stream().map(GetSubjectDto::new).collect(Collectors.toList())));
        } catch (NotFoundException e) {
            return status(NOT_FOUND).body(message(e));
        } catch (Exception e) {
            e.printStackTrace();
